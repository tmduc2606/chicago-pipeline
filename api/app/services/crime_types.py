...
async def get_top_types(
    ...
    return [CrimeTypeCount(primary_type=str(r.primary_type), count=int(r.count)) for r in rows]  # type: ignore[call-overload]
...
@cached(ttl=300)
async def get_multi_type_trend(
    db: AsyncSession,
    types: str,
    from_date: str | None = None,
    to_date: str | None = None,
    ...
    placeholders = ", ".join(f":trend_type_{i}" for i in range(len(type_list)))
    conditions.append(f"o.primary_type IN ({placeholders})")
    for i, t in enumerate(type_list):
        params[f"trend_type_{i}"] = t

    where = f"WHERE {' AND '.join(conditions)}"

    sql = text(f"""
        SELECT o.primary_type, t.date AS ts,
               COUNT(*)::int AS count,
               SUM(CASE WHEN f.is_arrested THEN 1 ELSE 0 END)::int AS arrests
        FROM {SCHEMA}.fact_crime f
        JOIN {SCHEMA}.dim_time t ON f.time_id = t.time_id
        JOIN {SCHEMA}.dim_offense o ON f.offense_id = o.offense_id
        {where}
        GROUP BY o.primary_type, t.date
        ORDER BY o.primary_type, t.date
    """)
    result = await db.execute(sql, params)
    rows = result.fetchall()
    return [
